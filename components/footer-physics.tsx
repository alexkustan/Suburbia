...
type FooterPhysicsProps = {
  ...
  className?: string;
  boardSize?: { width: number; height: number }; // Optional prop to pass board size
};

export function FooterPhysics({
  boardTextureURLs = [],
  className,
  boardSize = { width: 40, height: 120 }, // Smaller board size default
}: FooterPhysicsProps) {
  ...
  useEffect(() => {
    ...
    const boards = limitedBoardTextures.map((texture) => {
      ...
      return Bodies.rectangle(x, y, boardSize.width, boardSize.height, {
        chamfer: { radius: 40 },
        angle: rotation,
        restitution: 0.8,
        friction: 0.005,
        render: {
          sprite: {
            texture,
            xScale: boardSize.width / 200,
            yScale: boardSize.height / 600,
          },
        },
      });
    });
  ...
  }, [limitedBoardTextures, inView, boardSize]);
